...
class ULIPWithImageLoss(nn.Module):
    ...
    def forward(self, outputs, level):
        pc_embed = outputs['pc_embed']
        text_embed = outputs['text_embed']
        if level == 1 or level == 4:
            image_embed = outputs['image_embed']
        logit_scale = outputs['logit_scale'].unsqueeze(1)
        ...
        # else:
            # pc_embed_all, text_embed_all = utils.all_gather_batch([pc_embed, text_embed])

        # cosine similarity as logits
        # print("logit_scale.dtype", logit_scale.dtype)
        # print("logit_scale.shape", logit_scale.shape)
        # print("pc_embed.dtype", pc_embed.dtype)
        # print("text_embed.dtype", text_embed.dtype)
        # print("pc_embed.shape", pc_embed.shape)
        ...
        # print("logits_per_pc_text.dtype", logits_per_pc_text.dtype)
        # print("logits_per_text_pc.dtype", logits_per_text_pc.dtype)
        # print("logits_per_pc_text.shape", logits_per_pc_text.shape)
        # print("logits_per_text_pc.shape", logits_per_text_pc.shape)
        if level == 1 or level == 4:
            ...
            # print("logits_per_pc_image.dtype", logits_per_pc_image.dtype)
            # print("logits_per_pc_image.shape", logits_per_pc_image.shape) 
            # print("logits_per_image_pc.dtype", logits_per_image_pc.dtype)
            # print("logits_per_image_pc.shape", logits_per_image_pc.shape)
